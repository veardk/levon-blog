...
public class LeBlogTagController {
    ...
    @GetMapping("{id}")
    @SystemLog("查询标签详细")
    public ResponseResult getTagDetail(@PathVariable Long id) {
    ...
    @PostMapping
    @SystemLog("新增标签")
    public ResponseResult createTag(@RequestBody TagCreateValidationDTO createRequest) {
    ...
    @PutMapping
    @SystemLog("修改标签")
    public ResponseResult updateTag(@RequestBody TagUpdateValidationDTO updateRequest) {
    ...
    public ResponseResult deleteTags(@PathVariable("tagIds") String tagIds) {
        List<Long> ids = Arrays.stream(tagIds.split(","))
                .map(Long::parseLong)
                .collect(Collectors.toList());

        if (ids.isEmpty()) {
            return ResponseResult.errorResult(AppHttpCodeEnum.SYSTEM_ERROR, "No IDs provided for deletion.");
        }
        if (ids.size() == 1) {
